...
describe('crédit fonds de travailleurs dans le couple', () => {
  it('applique le crédit de 30 % au conjoint qui cotise (en plus de la déduction REER)', () => {
    const p1 = (partiel: Partial<PersonneProjection>): Partial<PersonneProjection> => ({
      ...
      epargneAnnuelle: { REER: 5_000 },
      comptes: [{ type: 'REER', solde: 0, profil: 'equilibre', rendementPersonnalise: 0 }], ...partiel,
    });
    // Même cotisation REER (5 000 $) dans les deux ; seule différence = le crédit du fonds (30 %).
    const sans = projeterCouple(couple(p1({}), { ageRetraite: 65 }, { inflation: 0, fraisGestion: 0 }));
